...
radioButtons("composition", tags$b("Composition"), 
...
![Composition [@RadAssist]](./images/composition.png){#fig-composition width="80%"}
...
radioButtons("echogenicity", tags$b("Echogenicity"), 
...
radioButtons("shape", tags$b("Shape"), choices = tirads_choices$shape, 
             selected = character(0),
             width = width_choices)
```
...
radioButtons("margin", tags$b("Margin"), choices = tirads_choices$margin,
             selected = character(0),
             width = width_choices)
```
...
```{r}
numericInput("size_cm", tagList(tags$b("Size"), "(cm)" ), value = NA, min = 0)
```
